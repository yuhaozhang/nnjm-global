...
class ModelGlobalPlus(object):
    def __init__(self, local_context_size, in_vocab_size, emb_dim, hidden_sizes, act_func, n_out, 
        num_section, model_parameters, fix_emb, global_nonlinear, adaptive_split):
    ...
    def buildModel(self):
        ...
        self.x = T.matrix('x')
        self.sm = T.matrix('sm')
        self.classifier = GlobalPlusNNLM(self.x, self.sm, self.activation, self.in_vocab_size, self.emb_dim, self.hidden_sizes, self.n_out, self.local_context_size, 
            self.num_section, self.model_parameters, self.fix_emb, self.global_nonlinear, self.adaptive_split)
        self._initSGD()
...
class GlobalPlusNNLM(object):
    ...
    def __init__(self, input_ngram, input_sm, activation, in_vocab_size, emb_dim, hidden_sizes, n_out, local_context_size, 
        ...
        if adaptive_split == False:
            self.linearLayer = GlobalPlusLinearLayer(input_ngram, input_sm, in_vocab_size, emb_dim, num_section, 
                linear_W_emb=linear_W_emb, fix_emb=fix_emb, nonlinear=global_nonlinear, activation=activation)
        else:
            self.linearLayer = GlobalPlusLinearLayerWithAdaptiveSplit(input_ngram, input_sm, in_vocab_size, emb_dim, num_section, 
                linear_W_emb=linear_W_emb, fix_emb=fix_emb, nonlinear=global_nonlinear, activation=activation)
        ...
        for ii in xrange(len(hidden_sizes)):
